...
test('Windows Codex automation has project, thread and safe new-chat routes', () => {
  ...
  assert.match(commands, /#\[cfg\(target_os = "windows"\)\]\s*#\[tauri::command\]\s*pub async fn open_new_codex_chat\(project_path: String\)/)
  assert.match(commands, /build_codex_project_new_thread_deeplink\(normalized_project_path\)/)
  assert.match(commands, /"codex:\/\/new\?path=\{\}"/)
  ...
  assert.match(appContent, /navigator\.platform\.toUpperCase\(\)\.includes\('WIN'\)[\s\S]*?invoke\('open_new_codex_chat', \{ projectPath \}\)[\s\S]*?return/)
