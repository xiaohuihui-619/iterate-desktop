...
test('Windows Codex automation has project, thread and safe new-chat routes', () => {
  ...
  assert.doesNotMatch(commands, /ShellExecuteW/)
  ...
  assert.match(commands, /#\[cfg\(target_os = "windows"\)\]\s*#\[tauri::command\]\s*pub async fn open_new_codex_chat\(thread_id: String\)/)
  assert.match(commands, /fn post_new_task_keypress_to_codex\(\)[\s\S]*?VK_CONTROL[\s\S]*?const VK_N: u8 = b'N'[\s\S]*?keybd_event\(VK_N/)
  assert.match(commands, /codex_thread_deeplink\(&thread_id\)[\s\S]*?launch_codex_desktop_deeplink\(&deeplink\)\?[\s\S]*?from_millis\(300\)[\s\S]*?post_new_task_keypress_to_codex\(\)/)
  assert.doesNotMatch(commands, /build_codex_project_new_thread_deeplink/)
  assert.doesNotMatch(commands, /codex:\/\/new\?path=/)
  ...
  assert.match(commands, /fn codex_desktop_app_candidates\(\)/)
  assert.match(commands, /var_os\("ProgramFiles"\)/)
  assert.match(commands, /\.join\("WindowsApps"\)/)
  assert.match(commands, /starts_with\("openai\.codex_"\)/)
  assert.match(commands, /\.join\("app"\)\.join\("ChatGPT\.exe"\)/)
  assert.match(commands, /fn resolve_codex_desktop_app_exe\(\)/)
  assert.match(commands, /Get-AppxPackage -Name OpenAI\.Codex/)
  assert.match(commands, /Select-Object -First 1 -ExpandProperty InstallLocation/)
  assert.match(commands, /官方 Codex Desktop App 打开 deeplink 失败/)
  assert.match(commands, /BringWindowToTop, SetForegroundWindow, ShowWindow, SW_RESTORE/)
  assert.match(commands, /SetForegroundWindow\(raw_hwnd\)/)
  assert.match(popup, /scheduleInputFocus\('request-visible', \{ activateWindow: true \}\)/)
  assert.match(popup, /options\.activateWindow \? 450 : 180/)
  assert.match(appContent, /const codexThreadId = normalizeCodexThreadId\(props\.mcpRequest\)[\s\S]*?if \(windowsPlatform && !codexThreadId\)[\s\S]*?if \(!windowsPlatform && !projectPath\)[\s\S]*?if \(windowsPlatform\)[\s\S]*?invoke\('open_new_codex_chat', \{ threadId: codexThreadId \}\)[\s\S]*?return/)
  assert.doesNotMatch(appContent, /invoke\('open_new_codex_chat', \{ projectPath \}\)/)
...
test('Windows popup screenshot keeps the Mac capability with a native Windows capture path', () => {
  const setup = source('src/rust/app/setup.rs')
  const commands = source('src/rust/ui/commands.rs')
